...
const JoinEventDetailPage = () => {
    ...
    const { mutate: mutateFreeRegistration, isLoading: isLoadingFreeRegistration } = useMutation({
        ...
        onError: (error: any) => {
            checkForErrors(error?.response?.data, isInterConnected, "Something went wrong while registering the user in free event! place:JoinEventDetailPagepage.tsx", error.message);
        }
    })
    ...
    const { mutate: mutatePaidRegistration } = useMutation({
            ...
            const options = {
                ...
                handler: async function (response: any) {
                    ...
                    if (verifyRes.data.success) {
                        ...
                        console.log("something went wrong while payment verification. place: JoinEventDetailPagePage.tsx");
        ...
        onError: (error: any) => {
            checkForErrors(error?.response?.data, isInterConnected, "Something went wrong while registering the user in paid event! place:JoinEventDetailPagepage.tsx", error.message);
        }
    })
                        ...
                        <div>
                            <div className="relative h-[300px] md:h-[400px]">
                                ...
                                <div className="absolute bottom-0 left-0 right-0 p-6 text-secondary">
                                    <h1 className="text-3xl md:text-4xl font-bold mb-2">{eventData?.data?.name}</h1>
